...
{
public:
    FragmentHighlighter(QTextDocument *parent)
...
protected:
    ...
    virtual void highlightBlock(const QString &text)
    {
        ...
        if(text.left(8).endsWith("#version")) {
            ...
            } else { myversion = GLSL_MIN_VERSION; }
        } else { myversion = lastversion; }

        QString verString = contextRules.value("version").value(QString("%1").arg(myversion));
        if(verString == "#pop") verString = QString("v%1%2").arg(myversion).arg( (myversion == 100 || myversion == 300) ? "ES":"" );
        else if(verString.isEmpty()) myversion = GLSL_MIN_VERSION;
        ...
        foreach (const HighlightingRule &rule, highlightingRules) {
            ...
            while(index != -1) { // found a word
                ...
                if(formatKey.isEmpty()) formatKey = rule.group; // probably not found glsl.xml?
	      ...
	      if(rule.group == "Preprocessor") break;
	    }
        }

        if(myversion != lastversion) glslVersionChanged = true;
        else glslVersionChanged = false;
    ...
    void setupFragmentariumDefaults()
    {
        ...
        patterns  << "\\bdonotrun\\b" << "\\bvertex\\b" << "\\bendvertex\\b" << "\\bbuffer\\b"
                  << "\\binfo\\b" << "\\bcamera\\b" << "\\bgroup\\b" << "\\bif\\b" << "\\bifdef\\b"
                  << "\\belse\\b" << "\\bendif\\b" << "\\bdefine\\b" << "\\binclude\\b"
                  << "\\bpreset\\b" << "\\bendpreset\\b" << "\\bTexParameter\\b" << "\\bbuffershader\\b"
                  << "\\bversion\\b" << "\\bundef\\b" << "\\bifndef\\b" << "\\belseif\\b"
                  << "\\berror\\b" << "\\bpragma\\b" << "\\bline\\b";
        ...
        while (!child.isNull())  {
            ...
            if (child.tagName() == "itemData")  {
                ...
                if( !(child.attribute("color").isEmpty()) ) {
                    ...
                    if(name == "Normaltext") myformat.setForeground(QBrush(Qt::black));
                    else if(name == "Keywordstypes") myformat.setForeground(QBrush(Qt::darkBlue));
                    else if(name == "Keyword") myformat.setForeground(QBrush(Qt::darkMagenta));
                    else if(name == "Reserved") myformat.setForeground(QBrush(Qt::black));
                    else if(name == "Buildinvariables") myformat.setForeground(QBrush(Qt::darkMagenta));
                    else if(name == "Buildconstants") myformat.setForeground(QBrush(Qt::darkMagenta));
                    else if(name == "Buildinfunctions") myformat.setForeground(Qt::darkYellow);
                    else if( (name == "Float") || (name == "Octal") || (name == "Hex") || (name == "Decimal") )
                        myformat.setForeground(QBrush(Qt::darkYellow));
                    else if(name == "Preprocessor") myformat.setForeground(Qt::blue);
                    else if(name == "Comment") myformat.setForeground(Qt::darkGreen);
                }
