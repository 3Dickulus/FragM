...
void VideoDialog::on_startButton_clicked()
{
    ...
    if (QFile::exists(fileName)) {
        if (QMessageBox::question(this, tr("Video"),
                                  tr("A file called %1 already exists in "
    ...
    if(program.contains("mencoder",Qt::CaseInsensitive) && !pm.isNull()) {
        ...
        arguments << QString("-mf w=%1:h=%2:fps=%3:type=%4").arg(pm.width()).arg(pm.height()).arg(mainWin->renderFPS).arg(filesuf);
        arguments << options;
...
void VideoDialog::encodingFinished()
{
    ...
    if (QFile::exists(fileName)) {
        m_ui->transcodingStatusLabel->setText(
          QString("Transcoding Status: %1")
          .arg( encstat ? "Success!" : "Failed!"));
    }
    else {
