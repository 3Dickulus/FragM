...
namespace SyntopiaCore {
    namespace Misc {
        ...
        MiniParser& MiniParser::getDouble(double& val) {
            ...
            if (!succes) {
                WARNING(QString("Expected argument number %1 to be a double. Found: %2").arg(paramCount).arg(first));
            }
            val = d;
        ...
        MiniParser& MiniParser::getFloat(float& val) {
            ...
            if (!succes) {
                WARNING(QString("Expected argument number %1 to be a float. Found: %2").arg(paramCount).arg(first));
            }
            val = d;
