...
// http://www.fractalforums.com/index.php?topic=16963.msg76553#msg76553
// double angle wrt positive-x axis
vec3 zpow2(vec3 c)
{
  float x = (c.x + c.y)*(c.x - c.y);
  float y = c.x * c.y;
  float z = c.z * c.z;
  return normalize(vec3(x, y+y, z+z));
}

uniform vec3 K;slider[(-2,-2,-2),(0,0,0),(2,2,2)];
uniform float domainMapScale; slider[-2,0,2]

vec3 domainMap(vec3 c)
{
if(domainMapScale != 0.0)
  c = domainMapScale*c;             // only effects resulting scale
